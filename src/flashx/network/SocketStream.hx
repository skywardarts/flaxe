...
class SocketStream
{
	...
	var socket:flashx.network.Socket;
	...
	public function new()
	{
		...
		this.add_message_handler(new flashx.network.events.KeepAliveEvent());
		this.add_message_handler(new flashx.network.events.LoginEvent());
		this.add_message_handler(new flashx.network.events.CreatePlayerEvent());
		this.add_message_handler(new flashx.network.events.RemovePlayerEvent());
		this.add_message_handler(new flashx.network.events.UpdatePlayerPositionEvent());
		this.add_message_handler(new flashx.network.events.UpdatePlayerRotationEvent());
	}
	...
	public function connect(host:String, port:Int, successful:Dynamic, failure:Dynamic):Void
	{
		...
		this.socket = new flashx.network.Socket(host, port);
		
		this.socket.add_event_handler(flashx.network.Socket.receive_successful_event, function(ba:flash.utils.ByteArray):Void
		{
			...
			while(processing)
			{
				...
				if(self.data.bytesAvailable >= 8)
				{
					...
					if(available > 0 && message_length > 0 && available >= message_length)
					{
						...
						var event:flashx.network.events.SocketEvent = self.message_handler_list[message_id].clone();
	...
	public function send_message(message:flashx.network.events.SocketEvent, successful:Dynamic, failure:Dynamic):Void
	{
	...
	public inline function add_message_handler(event:flashx.network.events.SocketEvent):Void
	{
		this.message_handler_list[event.id] = event;
	}
	
	public inline function add_event_handler(event:flashx.network.events.SocketEvent, handler:Dynamic):Void
	{
